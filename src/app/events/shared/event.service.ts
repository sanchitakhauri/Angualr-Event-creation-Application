...
export class EventService{
  ...
  constructor(private http: HttpClient){
    ...
    getEventList(): Observable<IEvent[]>{
      ...
      // setTimeout(() => {
      //   subject.next(event);subject.complete();
      // }, 100);
      //   return subject;
    ...
    saveEvent(eventNow){
      ...
      return this.http.post<IEvent>('/api/events', eventNow, options).pipe(catchError(this.handleError<IEvent>('saveEvent')));
    }

    // updateEvent(sessionEvent){
    //   let index = event.findIndex(x => x.id == sessionEvent.id)
...
// event can be deleted if you want
const event: IEvent[]  = [
  {
    id: 1,
